...
contract CryptomonContract {
    ...
    function catchCryptomon() public {
        Cryptomon memory _cmon = generateRandomCryptomon(random());
        if (cryptomonInWorld < 1) {
            initAttacks();
        }
    ...
    function playTurn(uint fightId, uint playerOneAction, uint playerTwoAction) public returns(uint) {
        ...
        battlegrounds[fightId].comment = string(abi.encodePacked(battlegrounds[fightId].playerOneCmon.name, deals, helpers.uintToString(playerOneAttack), damagesTo, battlegrounds[fightId].playerTwoCmon.name));
        ...
        battlegrounds[fightId].comment = string(abi.encodePacked(battlegrounds[fightId].comment, ', ', battlegrounds[fightId].playerTwoCmon.name, deals, helpers.uintToString(playerTwoAttack), damagesTo, battlegrounds[fightId].playerOneCmon.name));
